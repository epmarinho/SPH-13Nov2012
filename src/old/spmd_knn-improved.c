...
{
    double          r;
    if (Octree[cluster].identifier) {
        r = query2point_distance(x[query], x[Octree[cluster].identifier]);
        if (KNN_population < K && r < KNN[query].radius)
            KNN_add(query, Octree[cluster].identifier, r);
        else if (r < KNN[query].radius)
            KNN_insert(query, Octree[cluster].identifier, r);
...
do_knn_search(int n)
{
    ...
    for (i = 0; i < n; i++) {
        KNN[i].radius =
            Octree[1].size * pow((double) K / n, .333333333333333333333);
        knn_search(i);
    }
}
